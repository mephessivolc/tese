...
As equações abaixo para as \autoref{sec:gaussians_gate} e \autoref{sec:non_gaussians_gate} foram obtidas de \textcite{de_lima_redes_nodate} e ... 

\subsubsection{Portas Gaussianas} \label{sec:gaussians_gate}
Unitárias de primeira e segunda ordem
...
\paragraph*{Rotação $(R(\phi))$} Gira o estado de espaço de fase por um ângulo $\phi$. Corresponde a evolução livre do oscilador hamônico, sendo também a única com um análogo ao qubit, e que quando aplicada em estado de vácuo, essa operação não apresenta efeito, porém, sua aplicação em um estado coerente resulta em uma rotação de $\phi$ do estado em torno do centro do espaço de fase. Sua representação é dada por \autoref{eq:rotation_continuous_variables}

\begin{equation}
    \label{eq:rotation_continuous_variables}
    R(\phi)=\exp\left[i\frac{\theta}{2}\left(\hat(x)^2+\hat{p}^2-\mathrm{I}\right)\right]
\end{equation}

onde $\mathrm{I}$ é a matriz identidade de ordem igual a $\hat{x}$ e $\hat{p}$.

\paragraph*{Compressão ($S(r,\phi)$)} Comprime a incerteza de uma quadratura (por exemplo $\hat{x}$) enquanto expande a outra ($\hat{p}$), mantendo o Princípio da Incerteza, é uma porta que gera emaranhamento. A expressão da compressão é dada na equação \autoref{eq:squeezing_continuous_variables}

\begin{equation}
    \label{eq:squeezing_continuous_variables}
    S(r) = \exp\left[ \frac{r}{2}\left(e^{-i\phi}\hat{a}^2-e^{i\phi}\hat{a}^{\dagger^2}\right)\right]
\end{equation}
onde $r\geq 0$ e fase $\phi\in [0,2\pi]$.


\paragraph*{Divisão de Feixe $(BS(\mu,\phi))$} Combina dois modos bosônicos, usada para criar interferência e emaranhamento entre feixes de luz, onde a operação é dada pela \autoref{eq:beam_splitter_continuous_variables}

\begin{equation}
    \label{eq:beam_splitter_continuous_variables}
    BS_{i,j}(\mu,\phi)=\exp\left[\mu\left(e^{i\phi}\hat{a}_i\hat{a}_j^\dagger-e^{-i\phi}\hat{a}_i\hat{a}_j^\dagger\right) \right]
\end{equation}
...
Transforma a quadratura do momento com base no cubo da posição $\hat{p} \rightarrow \hat{p}+\gamma\hat{x}^2$, sendo extremamente difícil de implementar experimentalmente. A sua representação é dada na \autoref{eq:cubic_continuous_variables}
...
Uma não-linearidade onde o índice de refração depende da intensidade da luz, sendo representada pelo Hamiltoniano na \autoref{eq:kerr_continuous_variables}

\begin{equation}
    \label{eq:kerr_continuous_variables}
    H=\chi(\hat{a}^\dagger\hat{a})^2    
\end{equation}
