...
{\color{red}$<$Modificar$>$} Qubits fotônicos utilizam fótons (partículas de luz) para representar estados quânticos, codificando a informação em propriedades como polarização, fase, caminho ou em estados squeezed de campo eletromagnético. Eles têm a vantagem de operar em temperatura ambiente e de se propagarem por longas distâncias sem decoerência, o que os torna ideais para comunicações quânticas. Em termos de computação, os qubits fotônicos podem ser implementados via circuitos ópticos integrados, utilizando fontes de fótons entrelaçados, interferômetros reconfiguráveis e detectores de fóton único. Embora a tecnologia fotônica seja promissora (especialmente para escalabilidade modular), manipular e entrelaçar fótons de forma controlada é um grande desafio, e apenas nos últimos anos surgiram os primeiros protótipos de computadores fotônicos disponíveis para uso público \cite{slussarenko2019photonic}. A Xanadu, startup sediada em Toronto (Canadá), é líder em computação quântica fotônica. Seus processadores operam com o princípio de variáveis contínuas usando estados de luz squeezed (com ruído de vácuo reduzido) que são entrelaçados via arranjos de interferômetros óticos programáveis. Em junho de 2022, a Xanadu lançou o Borealis, um computador quântico fotônico de 216 qubits (estados squeezed) disponível publicamente na nuvem. A Quandela é uma startup francesa emergida de pesquisas do CNRS e da Universidade Paris-Saclay, focada em computação quântica fotônica. Ela desenvolveu fontes de fóton único altamente eficientes e, a partir de 2020, passou a construir seus próprios processadores fotônicos integrados. Em novembro de 2022, a Quandela lançou a primeira oferta de computação quântica na nuvem da Europa, disponibilizando acesso online a um chip fotônico de até 5 qubits naquela ocasião

{\color{red}$<$Modificar$>$} Qubits de íon aprisionado são implementados aprisionando íons (átomos carregados) em campos eletromagnéticos e manipulando seus estados internos com lasers. Essa tecnologia se destaca por longos tempos de coerência e altas fidelidades de porta, além de conectividade total entre qubits, embora as operações lógicas sejam relativamente lentas. A seguir, listamos empresas e instituições que utilizam íons aprisionados e disponibilizam esses sistemas para uso público via nuvem ou programas de acesso aberto \cite{bruzewicz_trapped-ion_2019}. IonQ é uma empresa comercial pioneira em computação quântica com íons aprisionados. Seus processadores usam íons de itérbio em armadilhas eletromagnéticas e apresentam conectividade all-to-all (todos os qubits totalmente conectados entre si). O sistema IonQ Aria, disponível desde 2022, possui 25 qubits físicos totalmente conectados e alcança 23–25 qubits algorítmicos (\#AQ), que é uma métrica de qubits “úteis” considerando mitigação de erros. A Quantinuum (resultado da fusão da Honeywell Quantum Solutions com a Cambridge Quantum) opera computadores quânticos de íons aprisionados com foco em alta fidelidade e melhoria contínua de hardware. Seu System Model H1 (primeira geração, derivado da Honeywell) foi disponibilizado comercialmente desde 2020 com 10–12 qubits totalmente conectados, posteriormente atualizado para 20 qubits totalmente conectados em 2022. 

{\color{red}$<$Modificar$>$} Qubits supercondutores do tipo transmon (um tipo de qubit de Josephson) são atualmente a abordagem mais difundida em hardware quântico. Esses qubits consistem em circuitos supercondutores operando em temperaturas criogênicas (~15 mK) e são controlados por pulsos de micro-ondas. Grandes empresas de tecnologia e startups adotaram transmons devido à sua facilidade de fabricação com técnicas de microeletrônica e velocidade de operação de portas lógicas \cite{koch2007charge}. A IBM é pioneira em computação quântica supercondutora e foi a primeira a oferecer acesso público a um processador quântico real em 2016. Os qubits da IBM são do tipo transmon (circuitos Josephson), integrados em processadores mantidos em temperaturas criogênicas. Atualmente, a IBM disponibiliza uma família de processadores supercondutores variando de pequenos dispositivos (5 e 7 qubits) até processadores de mais de 100 qubits. Usuários do mundo todo podem se cadastrar na plataforma IBM Quantum Experience (IBM Cloud) para rodar algoritmos gratuitamente em processadores de poucos qubits – por exemplo, sistemas de 5 qubits (ibmq\_lima) e 7 qubits (ibmq\_jakarta) são comumente oferecidos para experimentação aberta. Além disso, a IBM vem rompendo barreiras de escala: em 2021 apresentou o chip Eagle com 127 qubits (marcando a primeira vez que um processador superou 100 qubits). O Google, através de seu grupo Quantum AI, também utiliza qubits supercondutores (design próprios do tipo “Xmon”, uma variante do transmon). O Google não oferece acesso público direto a suas máquinas; em vez disso, foca em marcos científicos usando seus processadores em colaborações fechadas. Em 2019, a empresa demonstrou uma vantagem quântica com o chip Sycamore de 53 qubits, ao executar em 200 segundos uma tarefa de amostragem que seria impraticável em supercomputadores clássicos.
...
{\color{red}$<$Modificar$>$} O QUBO é semelhante ao hamiltoniniao de Ising $\displaystyle H=-\sum_{i<j}J_{ij}s_{i}s_{j}-\sum_{i}h_is_{i}$ {\color{red}$<$inserir referencia bibliografica$>$}com as transformações matemáticas necessárias de forma: $\ldots$
